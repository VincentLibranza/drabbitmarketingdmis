...
export class LocalDB {
  ...
  static appendLog(UserID: string, Action: string, TableRef: string) {
    ...
    const newEntry = { 
      LogID: `LOG-${Math.floor(Math.random() * 1000000)}`, // Matches ERD
      UserID: UserID,                                      // Matches ERD
      Action: Action,                                     // Matches ERD
      Timestamp: new Date().toLocaleString(),             // Matches ERD
      TableRef: TableRef                                  // Matches ERD
    };
