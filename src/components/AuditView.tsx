...
export default function AuditView({ logs, onRefreshData }: AuditViewProps) {
  ...
  const filteredLogs = logs.filter(log => {
    const actionVal = log.Action || log.action || "";
    const usernameVal = log.UserID || log.userId || log.username || "";
    const tableRefVal = log.TableRef || log.tableRef || "";
    const matchesSearch = 
      actionVal.toLowerCase().includes(searchTerm.toLowerCase()) || 
      usernameVal.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesTable = tableFilter === "All" || tableRefVal === tableFilter;
        ...
        <div className="divide-y divide-slate-100 p-2">
          {filteredLogs.length === 0 ? (
            ...
            filteredLogs.map(log => {
              ...
              const logIdVal = log.LogID || log.logId || "";
              const actionVal = log.Action || log.action || "";
              const usernameVal = log.UserID || log.userId || log.username || "";
              const tableRefVal = log.TableRef || log.tableRef || "";
              ...
              return (
                <div key={logIdVal} className="p-3.5 hover:bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs rounded-xl transition-colors font-sans">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-3 text-[10px] text-slate-400">
                        <span className="inline-flex items-center gap-1 font-medium">
                          ...
                          Actor Username: <strong className="text-slate-600 font-semibold">{usernameVal}</strong>
                        </span>
                        <span>•</span>
                        <span className="font-mono">Ref Code: {logIdVal}</span>
                      </div>
                    </div>
                  </div>

                  <div className="flex md:flex-col items-start md:items-end justify-between shrink-0 gap-1.5">
                    <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold font-mono tracking-wide ${getBadgeColor(tableRefVal)}`}>
                      {tableRefVal}
                    </span>
